"""
This module contains all of ppt's built-in commands. They are invoked when you
run `ppt <command>` on the command line. But you are also free to import them in
your Python build script and execute them there.
"""
from ppt import SETTINGS, activate_profile
from ppt.builtin_commands._util import (
    prompt_for_value,
    is_valid_version,
...
from ppt.upload import _upload_repo
from ppt.error import PbtError
from ppt.platform import (
    is_windows,
    is_mac,
    is_linux,
    is_arch_linux,
...
from ppt._variables import get_version, set_version
...
def freeze(debug=False):
    ...
    if is_mac():
        from ppt.freeze.mac import freeze_mac
        ...
        if is_windows():
            from ppt.freeze.windows import freeze_windows

            freeze_windows(debug=debug)
            executable += ".exe"
        elif is_linux():
            if is_ubuntu():
                from ppt.freeze.ubuntu import freeze_ubuntu

                freeze_ubuntu(debug=debug)
            elif is_arch_linux():
                from ppt.freeze.arch import freeze_arch

                freeze_arch(debug=debug)
            elif is_fedora():
                from ppt.freeze.fedora import freeze_fedora

                freeze_fedora(debug=debug)
            else:
                from ppt.freeze.linux import freeze_linux
...
def sign():
    ...
    require_frozen_app()
    if is_windows():
        from ppt.sign.windows import sign_windows
        ...
        _LOG.info("ppt does not yet implement `sign` on macOS.")
    else:
...
def installer():
    ...
    linux_distribution_not_supported_msg = (
        ...
        "You can run `ppt buildvm` followed by `ppt runvm` to start a Docker "
    ...
    if is_windows():
        from ppt.installer.windows import create_installer_windows

        create_installer_windows()
    elif is_mac():
        from ppt.installer.mac import create_installer_mac
        ...
        if is_ubuntu():
            ...
            from ppt.installer.arch import create_installer_arch

            create_installer_arch()
            install_cmd = "sudo pacman -U " + out_file
            remove_cmd = "sudo pacman -R " + app_name
        elif is_fedora():
            from ppt.installer.fedora import create_installer_fedora
...
def sign_installer():
    ...
    """
    if is_mac():
        _LOG.info("ppt does not yet implement `sign_installer` on macOS.")
        return
    if is_ubuntu():
    ...
    require_installer()
    if is_windows():
        from ppt.sign_installer.windows import sign_installer_windows

        sign_installer_windows()
    elif is_arch_linux():
        from ppt.sign_installer.arch import sign_installer_arch

        sign_installer_arch()
    elif is_fedora():
        from ppt.sign_installer.fedora import sign_installer_fedora
...
def repo():
    ...
    try:
        ...
        raise PbtError(
            ...
            "either\n"
            "    1) run `ppt gengpgkey` or\n"
            f'    2) set "gpg_key" and "gpg_pass" in {get_build_system_dir()}/build/settings/.'
        )
    if is_ubuntu():
        from ppt.repo.ubuntu import create_repo_ubuntu
        ...
        from ppt.repo.arch import create_repo_arch
        ...
        assert is_fedora()
        from ppt.repo.fedora import create_repo_fedora
...
def upload():
    """
    Upload installer and repository to ppt.sh
    """
    require_existing_project()
    try:
...
def _has_windows_codesigning_certificate():
    assert is_windows()
    from ppt.sign.windows import _CERTIFICATE_PATH
