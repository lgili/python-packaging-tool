...
def _main():
    """
    Main entry point for the `pbt` command line script.

    We init logging here instead of in pbt.cmdline.main(...) because the latter
    can be called by projects using pbt, and it's bad practice for libraries to
