...
from pbt._state import COMMANDS
...
def main(project_dir=None):
    """
    This function is executed when you run `pbt ...` on the command line. You
    can call this function from your own build script to run pbt as if it were
    ...
    if project_dir is None:
        project_dir = getcwd()
    try:
        pbt.init(project_dir)
        # Load built-in commands:
        from pbt import builtin_commands
        from pbt.builtin_commands import _docker
        from pbt.builtin_commands import _gpg
        from pbt.builtin_commands import _account
        from pbt.builtin_commands import _licensing
...
def command(f):
    """
    Use this as a decorator to define custom pbt commands. For an example, see:
        https://build-system.fman.io/manual/#custom-commands
    """
...
def _get_cmdline_parser():
    # Were we invoked with `python -m pbt`?
    is_python_m_fbs = splitext(basename(sys.argv[0]))[0] == "__main__"
    if is_python_m_fbs:
        prog = "%s -m pbt" % basename(sys.executable)
    else:
        prog = None
    parser = ArgumentParser(prog=prog, description="pbt")
